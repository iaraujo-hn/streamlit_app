...
# except OSError:
#     st.error("Spacy model 'en_core_web_sm' not found. Please download it using 'python -m spacy download en_core_web_sm'.")
...
def group_and_compare(df1, df2, groupby_columns, selected_metrics):
    ...
    print("DataFrame Columns (df1):", df1.columns.tolist())
    print("DataFrame Columns (df2):", df2.columns.tolist())
...
# Ensure df1, df2, groupby_columns, and selected_metrics are defined
df1 = read_file(st.file_uploader("Upload File 1"))
df2 = read_file(st.file_uploader("Upload File 2"))

if df1 is not None and df2 is not None:
    df1, df2 = manual_edit_columns(df1, df2)

    # Define groupby_columns and selected_metrics
    groupby_columns = st.multiselect("Select columns to group by", df1.columns.tolist())
    selected_metrics = st.multiselect("Select metrics to compare", df1.columns.tolist())

    if groupby_columns and selected_metrics:
        results, discrepancies_found, significant_discrepancies = group_and_compare(df1, df2, groupby_columns, selected_metrics)
...
# # filepath: /C:/Users/iaraujo/OneDrive - HorizonMedia/Documents/Clients/Repositories/testing-tool/streamlit_app/pages/5_QA_Tool_Cleaned.py
# results, discrepancies_found, significant_discrepancies = group_and_compare(df1, df2, groupby_columns, selected_metrics)
